...
leaf_data <- read.csv("C:/Users/15404/Documents/GitHub/research_project/Final_Processing/Dataset_Creation/Datasets/leafPhylum_plantDNA.csv", header = TRUE, sep = ",")
...
tot_sum = colSums(leaf_data[, c(taxa)])
...
pdf(file="./Figures/leaf_micro_rel_abund_phyla.pdf", width = 15, height = 10)
...
test_col <- subset(leaf_data, site_id == unique(leaf_data$site_id)[1])
col_sums = colSums(test_col[, c(taxa)])
col_sums <- col_sums/(sum(col_sums))
site_df <- as.data.frame(col_sums)

for (site_x in unique(leaf_data$site_id)) {
  sub_df <- subset(leaf_data, site_id == site_x)
...
site_list <- df_long$Var1

bgr_list <- c()

for (ex_site in site_list){
  len <- length(bgr_list)
  bgr_list[[len+1]] <- (unique(leaf_data['BGR'][leaf_data['site_id'] == ex_site]))
}

df_long$BGR <- unlist(bgr_list)

class(df_long$BGR)


p <- ggplot(df_long, aes(x = Var1, y = value, fill = Var2)) +
...
edf <- subset(leaf_data, genetic_spec == 'Exaltata')
sdf <- subset(leaf_data, genetic_spec == 'Syriaca')
...
edf <- subset(leaf_data, binned_sp == 'Exaltata')
hdf <- subset(leaf_data, binned_sp == 'Hybrid')
sdf <- subset(leaf_data, binned_sp == 'Syriaca')
...
bdf <- subset(leaf_data, BGR == 'Blacksburg')
cdf <- subset(leaf_data, BGR == 'Cole Mountain')
wdf <- subset(leaf_data, BGR == 'Wintergreen')
...
bgr_plot
pdf(file="./Figures/leaf_micro_bgr_rel_abund_BC.pdf", width = 15, height = 10)
bgr_plot
dev.off()


pdf(file="./Figures/leaf_micro_sp_rel_abund_BC.pdf", width = 15, height = 10)
sp_plot
dev.off()


pdf(file="./Figures/leaf_micro_binned_rel_abund_BC.pdf", width = 15, height = 10)
binned_plot
dev.off()


pdf(file="./Figures/leaf_micro_site_rel_abund_BC.pdf", width = 15, height = 10)
site_plot
dev.off()
